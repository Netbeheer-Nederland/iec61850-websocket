...
async def main():
    ...
    ep_wsServer = WebSocketEndpoint(oauth_enable=True, cert_endpoint="https://192.168.100.15:8443/realms/master/protocol/openid-connect/certs",
        ...
        if websocket_info is not None:
            try:
                ...
                ds_directory = await ep_wsServer.client_list[0].get_dataset_directory("LD0", "LLN0", "DataSetMinMaxAvg",
                                                                       websocket_info, callback_called, None)
                da_def = await ep_wsServer.client_list[0].get_data_definition("LD0/LLN0.Mod", websocket_info, callback_called, None)
                da_val = await ep_wsServer.client_list[0].get_data_values("LD0/DWMX1.WMaxSetPct", "sp", True, websocket_info, callback_called, None)
                set_da_res = await ep_wsServer.client_list[0].set_data_values("LD0/DWMX1.WMaxSetPct", "sp", data_WMaxSetPct,
                                                                 websocket_info, callback_called, None)
