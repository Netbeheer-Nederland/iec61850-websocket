    ...
    if ep_wsServer.client_list[0].is_connected is True:
        ...
        if websocket_info is not None:
            try:
                ...
                select_result = await ep_wsServer.client_list[0].select("LD0/DWMX1.WMaxSetPct", websocket_info, None, None)
                print(select_result)

                operate_result = await ep_wsServer.client_list[0].operate(oper_val_wrong_type, websocket_info, None, None)
                print("result for incorrect data type in operate request", operate_result)

                operate_result = await ep_wsServer.client_list[0].operate(oper_val_incorrect_do, websocket_info, None, None)
                ...
                operate_result = await ep_wsServer.client_list[0].operate(oper_val_out_of_range, websocket_info, None, None)
