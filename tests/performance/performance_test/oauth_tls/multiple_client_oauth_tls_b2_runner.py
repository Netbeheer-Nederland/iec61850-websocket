...
for path in sys.path:
    if path.endswith("client_credentials"):
        ...
        break
credentials_file_path = os.path.join(credentials_path, 'client_credentials_500_2.json')
...
async def start_client_process(client_config, i):
    ...
    refresh_task = asyncio.create_task(
        refresh_token_if_needed(token_request_url, client_config['client_id'], client_config['client_secret'],
                                access_token, ep_wsClient, client_config['cp_id'], cert_path, keycloak_cert_path))
