...
def create_bff_router() -> tuple[APIRouter, ACSIClient]:
    ...
    def api_connect(request: ConnectRequest):
        ...
        try:
            ...
            port = request.port
            cp = request.cp.lower()
            raw_port = port.get("port", 8765)

            try:
                port = int(raw_port)
            except (TypeError, ValueError):
                return JSONResponse(content={"ok": False, "error":  f"Invalid port value: {raw_port!r}"}, status_code=400)
    ...
    def api_model():
        ...
        try:
            ...
            if loop is None or not getattr(loop, "is_running", lambda: False)():
                return JSONResponse(content={"ok": False, "error": "client-not-connected"}, status_code = 503)
            ...
            if status == 'error':
                return JSONResponse(content={'status': 'error', 'error': error}, status_code = 500)
            ...
            if status == 'idle':
                ...
                if start_result == 'error':
                    ...
                    return JSONResponse(content={'status': 'error', 'error': client.runtime.model_error}, status_code = 503)
            with client.runtime.lock:
            ...
            return JSONResponse(content={"ok": False, "error": str(exc), "traceback": tb}, status_code = 500)
    ...
    def api_write_value(request: WritevalueRequest):
        ...
        """
        try:
            obj_ref = WritevalueRequest.obj_ref
            fc = WritevalueRequest.fc
            value = WritevalueRequest.value
            value_type = WritevalueRequest.value_type
            ...
            if not fc:
                ...
                return JSONResponse(content={"ok": False, "error": "fc is required"}, status_code=400)
            ...
            if value is None:
                ...
                return JSONResponse(content={"ok": False, "error": "value is required"}, status_code=400)
            ...
            if client.runtime.client is None:
                ...
                return JSONResponse(content={"ok": False, "error": "Client is not connected"}, status_code=503)
            ...
            try:
                ...
                return JSONResponse(content={"ok": False, "error": "write timeout"}, status_code=504)
            except ValueError as exc:
    ...
    @router.get('/internal/model/status')
...
def create_fastapi_app()  -> FastAPI:
    ...
    async def log_all_requests(request: Request, call_next):
        """Middleware that logs ALL GET/POST requests using client._log_action"""
        ...
        # For POST requests, extract body parameters
        if method == "POST":
            try:
        ...
        # Log the request
        client._log_action(f"API {method} {path}", detail=detail)
