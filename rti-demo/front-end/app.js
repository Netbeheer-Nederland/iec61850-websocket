...
${JSON.stringify(result, null, 2)}</pre>`;
    ...
    updateScanConfigFromSettingsInputs(persist = false) {
        ...
        const raw   = (document.getElementById('scan-ports')?.value || '').trim();
        const ports = this.parsePortList(raw);
        if (!host || !ports.length) {
            this.addDiagnosticMessage('Invalid discovery settings — provide a host and at least one port.', 'error');
            return false;
        }
        this.scanHost  = host;
        this.scanPorts = ports.join(',');
        if (persist) { localStorage.setItem('scanHost', host); localStorage.setItem('scanPorts', this.scanPorts); }
        const el = document.getElementById('scan-ports');
        if (el) el.value = this.scanPorts;
        return true;
    }
    ...
    startAutoRefresh() {
        ...
        this.autoRefreshInterval = setInterval(() => {
            ...
            if (active && active.id === 'page-dashboard') this.refreshDashboard();
        }, 5000);
    }
