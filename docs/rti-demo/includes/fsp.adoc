== RTI-FSP: IEC 61850 ACSI Server - WS Active

This folder contains the **RTI-FSP (Flexibility Service Provider)** implementation - an **IEC 61850 ACSI Server** 
that provides WebSocket-based communication in active mode for substation automation and power system control.

=== Overview

The RTI-FSP directory implements a complete **IEC 61850 ACSI (Abstract Communication Service Interface) Server** that:
...
image::../images/RTI_FSP_ACSI_SERVER.png[RTI-FSP ACSI Server Architecture,link="../images/RTI_FSP_ACSI_SERVER.png"]
...
==== acsi_server.py - ACSI Server & WebSocket -Active Mode
...
==== Docker
----
docker build -t rti-demo-RTI-FSP -f examples/rti-demo/RTI-FSP/Dockerfile .
docker run --rm -p 5001:5001 rti-demo-RTI-FSP
# With network
docker network create rti-network
docker run --rm -p 5001:5001 --network rti-network --name rti-RTI-FSP rti-demo-RTI-FSP
----
...
* With SO: RTI-FSP (Server, Port 8765) <--WebSocket--> SO (Client)
