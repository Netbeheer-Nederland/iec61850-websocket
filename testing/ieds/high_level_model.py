...
from ws61850.iec61850.data_model.helper import create_apc_do, create_asg_do_custom, create_lpl_do, create_ens_do, \
    create_mv_do, create_wye_do, create_del_do, create_enc_do, create_dpl_do, create_sps_do, create_asg_do, \
    create_ing_do, create_inc_do
...
def make_ied_model1():
    ...
    data_entry_2 = DataSetEntry("LD0", False, "LD0/DWMX1.WMaxFto.setVal", -1, None, None, FunctionalConstraint.sp)
    data_entry_3 = DataSetEntry("LD0", False, "LD0/DWMX1.WMaxSet.setMag.f", -1, None, None, FunctionalConstraint.sp)
    data_entry_4 = DataSetEntry("LD0", False, "LD0/DWMX1.WMaxSetPct.setMag.f", -1, None, None, FunctionalConstraint.sp)
    ...
    brcb = ReportControl("LD0/LLN0.rcbMinMaxAvg", ln2, "rcbMinMaxAvg", "MinMaxAvg", True,
                         "LD0/LLN0.DataSetMinMaxAvg", 1, trgOpts, optFields, "None", 2000, None, False)

    ln2.add_reportControl(brcb)

    #URCB
    optFields = asdict(OptFldsRCB(seqNum=False, timeStamp=True, dataSet=False, reasonCode=True, dataRef=False, entryID=False,
                                  configRef=False, bufOvfl=False))
    #trgOpts = asdict(TrgOps(integrity=True))
    trgOpts = {"dchg": True, "qchg": False, "dupd": False, "integrity": True, "gi": True}

    urcb = ReportControl("LD0/LLN0.rcbSetpoints", ln2, "rcbSetpoints", "Setpoints", False,
                         "LD0/LLN0.DataSetSetpoints", 1, trgOpts, optFields, 100, 1000, None, False)

    ln2.add_reportControl(urcb)

    #URCB
    optFields = asdict(OptFldsRCB(seqNum=False, timeStamp=True, dataSet=False, reasonCode=True, dataRef=False, entryID=False,
                                  configRef=True, bufOvfl=False))
    #trgOpts = asdict(TrgOps(integrity=True))
    trgOpts = {"dchg": True, "qchg": True, "dupd": False, "integrity": False, "gi": True}

    urcb = ReportControl("LD0/LLN0.rcbActualValues", ln2, "rcbActualValues", "DataSetActualValues", False,
                         "LD0/LLN0.DataSetActualValues", 1, trgOpts, optFields, 1000, 1000, None, False)
