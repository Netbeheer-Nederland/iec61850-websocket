...
class RTILauncher:
    ...
    def parse_args(
    ...
    ) -> tuple[list[ServiceType], dict[str, str]]:
        ...
        parser.add_argument(
            "--port", type=int, help="Override default port for the service"
        )
        parser.add_argument(
            "--docker", action="store_true", help="Run services in Docker containers"
        )
        parser.add_argument(
            "--background",
            action="store_true",
        ...
        parser.add_argument(
            "--status", action="store_true", help="Show status of running services"
        )
        parser.add_argument(
            "--config",
            type=str,
    ...
    def _get_service_config(
        self, svc_type: ServiceType, port: int | None = None
    ) -> ServiceConfig:
        ...
        if port is not None:
            config = ServiceConfig(
                ...
                labels={**config.labels, "rti.port": str(port)}
                if config.labels
                else {},
            )
    ...
    def _launch_python_service(
        self, svc_type: ServiceType, config: ServiceConfig, background: bool = False
    ) -> subprocess.Popen:
    ...
    def _launch_docker_service(
        self, config: ServiceConfig, background: bool
    ) -> subprocess.Popen:
        ...
        cmd = [
            ...
            f"{port}:{port}",
            "--name",
            f"rti-{config.service_type.value}",
        ]
    ...
    def check_health(
        self, host: str = "localhost", timeout: int = 5
    ) -> dict[str, bool]:
    ...
    def save_config(
        self, config: dict, config_path: str = "config/launch_config.json"
    ) -> bool:
