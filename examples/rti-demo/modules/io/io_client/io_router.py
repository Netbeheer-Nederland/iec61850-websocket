...
def create_io_router() -> APIRouter:
    ...
    def _create_io_to_iec_callback(
        device_name: str,
        obj_ref: str,
        fc: str,
        ...
        async def callback(old_value: Any, new_value: Any):
            logger.info(
                f"Device {device_name}: {old_value} -> {new_value}, writing to {obj_ref}"
            )
            try:
                if service == "writeValue":
                    success = await io_client.write_to_iec61850(
                        obj_ref, new_value, fc, data_type
                    )
                    if success:
                        logger.info(
                            f"Successfully wrote {device_name}={new_value} to {obj_ref}"
                        )
                    else:
                        logger.warning(
                            f"Failed to write {device_name}={new_value} to {obj_ref}"
                        )
                else:  # operate
                    # For operate, use value_type parameter. If data_type is empty, use default "BOOLEAN"
                    if data_type:
                        success = await io_client.operate_to_iec61850(
                            obj_ref, new_value, value_type=data_type
                        )
                    else:
                        success = await io_client.operate_to_iec61850(
                            obj_ref, new_value
                    ...
                    if success:
                        logger.info(
                            f"Successfully operated {device_name}={new_value} to {obj_ref}"
                        )
                    else:
                        logger.warning(
                            f"Failed to operate {device_name}={new_value} to {obj_ref}"
                        )
    ...
    def _register_input_callback(
        ...
        if not controller or not io_client:
            logger.warning(
                "IO controller or client not available - input device callbacks require same-process operation"
            )
            return False
        ...
        io_to_iec_callback = _create_io_to_iec_callback(
            device_name, obj_ref, fc, io_client, service, data_type
        )
        device.register_change_callback(io_to_iec_callback)
        logger.info(
            f"Registered IEC61850 callback: {device_name} -> {obj_ref} (service={service}, dataType={data_type})"
        )
        return True
    ...
    async def api_io_health(request: Request):
        ...
        try:
            ...
            return {"status": "ok", "demo_io_healthy": True, "demo_io_info": health}
        except HTTPException:
            raise
        except Exception as exc:
            return {"status": "error", "demo_io_healthy": False, "error": str(exc)}
    ...
    @router.get(
        ...
        responses={200: {"description": "List of all mappings"}},
        tags=["IO Mapping"],
    )
    async def api_list_mappings(request: Request):
        ...
        try:
            ...
            return MappingListResponse(
                mappings=response_mappings, count=len(response_mappings)
            )
        except Exception as exc:
    ...
    async def api_remove_mapping(device_name: str, request: Request):
        ...
        try:
            ...
            return {"ok": True, "message": f"Mapping removed for {device_name}"}
        except HTTPException:
            raise
        except Exception as exc:
    ...
    async def api_clear_mappings(request: Request):
        ...
        try:
            ...
            return {"ok": True, "message": "All mappings cleared"}
        except Exception as exc:
    ...
    @router.get(
        ...
        responses={200: {"description": "List of objRefs"}},
        tags=["IO Mapping"],
    )
    async def api_list_objrefs(request: Request):
        ...
        try:
            ...
            return {"ok": True, "objRefs": objrefs, "count": len(objrefs)}
        except Exception as exc:
        ...
        try:
            ...
            try:
                response = await io_client._request(
                    "POST", "/acsi/sync-mappings", json={"mappings": server_mappings}
                )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to sync mappings to IO server: {e}"
                )
    ...
    async def api_enable_acsi_server_sync(request: Request):
        ...
        try:
            ...
            try:
                response = await io_client._request(
                    "POST", "/acsi/config", json={"url": acsi_url, "enabled": True}
                )
                ...
                if response.get("ok"):
                    ...
                    if server_mappings:
                        ...
                        logger.info(
                            f"Enabled ACSI sync on IO server and synced {len(server_mappings)} mappings"
                        )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to enable ACSI sync: {e}"
                )
