...
class PotentiometerConfig(DeviceConfig):
    ...
    change_threshold: float = (
        0.01  # Fraction of range to trigger change detection (default 1%)
    )
...
class LCDConfig(DeviceConfig):
    ...
    gpio_rs: int = 26
    gpio_e: int = 19
    gpio_data: list[int] = field(
        default_factory=lambda: [13, 12, 16, 20]
    )  # D4, D5, D6, D7
    gpio_rw: int | None = None  # RW pin (optional, tie to GND if not used)
    columns: int = 16
    rows: int = 2
    backlight: bool = True
    backlight_pin: int | None = None  # Optional backlight control pin
...
class IODevice(ABC):
    ...
    @abstractmethod
    def read(self) -> bool | float | None:
        pass

    @abstractmethod
    def write(self, value: bool | float) -> bool:
        pass
...
class _GpioZeroLED:
    ...
    def __init__(
        self, gpio_pin: int, active_high: bool = True, initial_value: bool = False
    ):
        self._lines = None
        self._chip = None
        ...
        if not GPOD_AVAILABLE:
            raise RuntimeError(
                "gpiod library not available. LED requires GPIO hardware."
            )
        ...
        try:
            ...
            initial_val = (
                gpiod.line.Value.ACTIVE if initial_value else gpiod.line.Value.INACTIVE
            )
            settings = gpiod.line_settings.LineSettings(
                direction=direction, active_low=active_low, output_value=initial_val
            )
            self._lines = self._chip.request_lines({gpio_pin: settings})
            logger.info(f"Initialized gpiod LED on GPIO {gpio_pin}")
        except Exception as e:
            raise RuntimeError(
                f"GPIO {gpio_pin} unavailable: {e}. LED requires GPIO hardware."
            )
...
class _GpioZeroButton:
    ...
    def __init__(
        ...
        if not GPOD_AVAILABLE:
            raise RuntimeError(
                "gpiod library not available. Button requires GPIO hardware."
            )
        ...
        try:
            ...
            active_low = not active_high
            bias = (
                gpiod.line_settings.Bias.PULL_UP
                if pull_up
                else gpiod.line_settings.Bias.PULL_DOWN
            )
            settings = gpiod.line_settings.LineSettings(
                direction=direction,
            ...
            raise RuntimeError(
                f"GPIO {gpio_pin} unavailable: {e}. Button requires GPIO hardware."
            )
...
class InputDevice(IODevice):
    ...
    def __init__(self, config: DeviceConfig):
        ...
        self._value_history: list[bool | float | None] = []
    ...
    def _start_polling_monitor(
        self, poll_interval: float = 0.1, threshold: float | None = None
    ) -> None:
        ...
        def monitor_loop():
            ...
            while not self._stop_monitoring:
                ...
                if current_value is not None:
                    ...
                    if self._poll_threshold is not None and isinstance(
                        current_value, (int, float)
                    ):
                        if self._last_reported_value is not None and isinstance(
                            self._last_reported_value, (int, float)
                        ):
                            if (
                                abs(current_value - self._last_reported_value)
                                >= self._poll_threshold
                            ):
...
class PotentiometerDevice(InputDevice):
    ...
    def __init__(self, config: PotentiometerConfig):
        ...
        self._adc_type = None
        self._channel = 0
        self._value_change_threshold = (
            config.change_threshold
        )  # Use config's threshold (as a fraction of the range)
        self._mock_value = 0.5  # Default midpoint value for mock mode (normalized 0-1)
        self._init_hardware()
        if not self._hardware_available:
            logger.warning(
                f"ADC not available for potentiometer '{config.name}'. Using mock mode."
            )
    ...
    def _init_hardware(self) -> None:
            ...
            try:
                ...
                if adc_type_config == "mcp3008":
                    try:
                        ...
                        logger.info(
                            f"Initialized potentiometer '{self.config.name}' on MCP3008 ADC channel {self.config.adc_channel}"
                        )
                        return
                    except (ImportError, AttributeError, OSError) as e:
                        logger.warning(
                            f"MCP3008 initialization failed for '{self.config.name}': {e}"
                        )
                        self._hardware_available = False
                ...
                if adc_type_config == "ads1115" or not self._hardware_available:
                    try:
                        ...
                        self._adc = Adafruit_ADS1x15.ADS1115(
                            address=self.config.i2c_address, busnum=self.config.i2c_bus
                        )
                        self._adc_type = self.ADC_ADS1115
                        self._channel = self.config.adc_channel
                        self._hardware_available = True
                        logger.info(
                            f"Initialized potentiometer '{self.config.name}' on ADS1115 channel {self.config.adc_channel} (bus={self.config.i2c_bus}, address=0x{self.config.i2c_address:02X})"
                        )
                        return
                    except (ImportError, AttributeError, OSError) as e:
                        logger.warning(
                            f"ADS1115 initialization failed for '{self.config.name}' (attempt {attempt + 1}/{max_retries}): {e}"
                        )
                        self._hardware_available = False
                ...
                logger.debug(
                    f"ADC initialization failed (attempt {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                else:
        ...
        if not self._hardware_available:
            logger.warning(
                f"No ADC hardware available for potentiometer '{self.config.name}'. Using mock mode."
            )
        self._hardware_available = (
            self._hardware_available or True
        )  # Allow operation in mock mode
    ...
    def read(self, scaled: bool = True) -> float | None:
        """
        ...
        try:
            ...
            if scaled:
                ...
                return self.config.min_value + normalized * (
        ...
        if self._adc_type is None:
            ...
            if value_range > 0:
                ...
                normalized_value = max(
                    0, min(1, (float(value) - self.config.min_value) / value_range)
                )
...
class ButtonDevice(InputDevice):
    ...
    def __init__(self, config: ButtonConfig):
        ...
        try:
                ...
                try:
                    ...
                    if self.config.is_active_high:
                        ...
                        self._last_reported_value = (
                            raw_value if config.latching else None
                        )
                    else:
                        self._previous_physical_state = not raw_value
                        self._last_reported_value = (
                            not raw_value if config.latching else None
                        )
                except Exception:
                    self._previous_physical_state = False
                    self._last_reported_value = None
            ...
            logger.info(
                f"Initialized button '{config.name}' on GPIO {config.gpio_pin}"
                + (" (latching)" if config.latching else "")
            )
        except Exception as e:
            raise RuntimeError(
                f"Hardware not available for button '{config.name}': {e}. Button requires GPIO hardware."
            )

    def _read_physical_raw(self) -> bool | None:
    ...
    def _get_physical_state(self) -> bool | None:
...
class LCDDevice(IODevice):
    ...
    def __init__(self, config: LCDConfig):
        ...
        if self._hardware_available:
            ...
            logger.info(
                f"Initialized LCD device '{config.name}' ({config.columns}x{config.rows}) on GPIO RS={config.gpio_rs}, E={config.gpio_e}, D4-D7={config.gpio_data}"
            )
        else:
            logger.warning(
                f"GPIO hardware not available for LCD '{config.name}'. Using mock mode."
            )
    ...
    def _init_hardware(self) -> None:
        ...
        try:
            ...
            self._gpio_lines = self._gpio_chip.request_lines(
                {pin: settings for pin in all_pins}
            )
    ...
    def _write4bits(self, value: int) -> None:
        ...
        try:
            ...
            for i, pin in enumerate(self._gpio_data):
                bit = (value >> i) & 0x01
                pin_values[pin] = (
                    gpiod.line.Value.ACTIVE if bit else gpiod.line.Value.INACTIVE
                )
    ...
    def _send_byte(self, value: int, mode: int) -> None:
        ...
        try:
            ...
            logger.info("[LCD SEND] RS reset to LOW")
        except Exception as e:
    ...
    def write(self, value: bool | float | str | list[str]) -> bool:
        """
        ...
        if not self._hardware_available:
            logger.warning(
                f"Cannot write to LCD '{self.config.name}': hardware not available"
            )
            return False
        ...
        try:
            ...
            logger.info("[LCD DEBUG] Write complete")
            return True
        except Exception as e:
    ...
    def write_line(self, line_number: int, text: str) -> bool:
        ...
        if line_number < 0 or line_number >= self._rows:
            logger.warning(
                f"Invalid line number {line_number} for LCD with {self._rows} rows"
            )
            return False
    ...
    def set_cursor(self, row: int, col: int) -> bool:
        ...
        if row < 0 or row >= self._rows or col < 0 or col >= self._columns:
            logger.warning(
                f"Invalid cursor position ({row}, {col}) for {self._columns}x{self._rows} LCD"
            )
            return False
...
class LCDI2CDevice(IODevice):
    ...
    def __init__(self, config: LCDI2CConfig):
        ...
        if self._hardware_available:
            self._initialize_display()
            logger.info(
                f"Initialized I2C LCD device '{config.name}' ({config.columns}x{config.rows}) at 0x{config.i2c_address:02X} on bus {config.i2c_bus}"
            )
        else:
            logger.warning(
                f"I2C hardware not available for LCD '{config.name}'. Using mock mode."
            )
    ...
    def _init_hardware(self) -> None:
        ...
        if not SMBUS2_AVAILABLE:
            logger.warning(
                "smbus2 library not available. I2C LCD requires smbus2. Install with: pip install smbus2"
            )
            self._hardware_available = False
            return
        ...
        try:
            ...
            logger.warning(
                f"I2C initialization failed for LCD '{self.config.name}': {e}"
            )
            self._hardware_available = False
    ...
    def write(self, value: bool | float | str | list[str]) -> bool:
        ...
        try:
            ...
            logger.info("[I2C LCD DEBUG] Write complete")
            return True
        except Exception as e:
    ...
    def write_line(self, line_number: int, text: str) -> bool:
        ...
        if line_number < 0 or line_number >= self._rows:
            logger.warning(
                f"Invalid line number {line_number} for I2C LCD with {self._rows} rows"
            )
            return False

        if not self._hardware_available:
            logger.debug(
                f"I2C LCD '{self.config.name}' in mock mode - write_line simulated"
            )
            return True
    ...
    def set_cursor(self, row: int, col: int) -> bool:
        ...
        if row < 0 or row >= self._rows or col < 0 or col >= self._columns:
            logger.warning(
                f"Invalid cursor position ({row}, {col}) for {self._columns}x{self._rows} I2C LCD"
            )
            return False

        if not self._hardware_available:
            logger.debug(
                f"I2C LCD '{self.config.name}' in mock mode - set_cursor simulated"
            )
            return True
    ...
    def display_on(self, state: bool = True) -> bool:
        ...
        if not self._hardware_available:
            logger.debug(
                f"I2C LCD '{self.config.name}' in mock mode - display_on simulated"
            )
            return True
...
def validate_device_config(config: DeviceConfig) -> bool:
    ...
    if config.device_type == DeviceType.LED:
        ...
        if config.gpio_pin not in RASPBERRY_PI_VALID_GPIO:
            raise ValueError(
                f"Invalid GPIO pin {config.gpio_pin}. Valid: {sorted(RASPBERRY_PI_VALID_GPIO)}"
            )
        ...
        if config.adc_channel not in MCP3008_VALID_CHANNELS:
            raise ValueError(
                f"Invalid ADC channel {config.adc_channel}. Valid: {sorted(MCP3008_VALID_CHANNELS)}"
            )
        ...
        for pin in all_pins:
            if pin not in RASPBERRY_PI_VALID_GPIO:
                raise ValueError(
                    f"Invalid GPIO pin {pin} for LCD. Valid: {sorted(RASPBERRY_PI_VALID_GPIO)}"
                )
        if config.columns not in [8, 16, 20, 24, 40]:
            raise ValueError(
                f"Invalid LCD columns {config.columns}. Valid: [8, 16, 20, 24, 40]"
            )
        ...
        if config.i2c_address < 0x08 or config.i2c_address > 0x77:
            raise ValueError(
                f"Invalid I2C address 0x{config.i2c_address:02X}. Valid range: 0x08-0x77"
            )
        ...
        if config.columns not in [8, 16, 20, 24, 40]:
            raise ValueError(
                f"Invalid I2C LCD columns {config.columns}. Valid: [8, 16, 20, 24, 40]"
            )
        if config.rows not in [1, 2, 4]:
