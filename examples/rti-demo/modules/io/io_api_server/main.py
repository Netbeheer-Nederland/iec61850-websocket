...
def load_acsi_config(config_path: Path) -> ACSIConfig | None:
...
def create_app() -> FastAPI:
    ...
    if config_loaded:
        ...
        logger.info(
            "Configured default devices: led1 (GPIO 17), led2 (GPIO 18), led3 (GPIO 22), pot1 (ADC 0), button1 (GPIO 10)"
        )
    ...
    if not io_controller.initialize():
        ...
        led_names = [
            ...
            if io_controller.get_config(name).device_type.value == "led"
        ]
        if led_names:
...
def main():
    ...
    if platform.system() == "Windows":
        host = "localhost"
        logger.info("Running on Windows - using host='localhost' for compatibility")
    else:
        host = "0.0.0.0"
