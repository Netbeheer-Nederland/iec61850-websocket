...
def test_2_config_creation():
    ...
    print(
        f"Bit mappings: rs={config.rs_bit}, rw={config.rw_bit}, e={config.e_bit}, bl={config.backlight_bit}"
    )
...
def test_3_config_validation():
    ...
    config = LCDI2CConfig(
        name="test_lcd_i2c", i2c_address=0x27, i2c_bus=1, columns=16, rows=2
    )
...
def test_5_device_factory():
    ...
    config = LCDI2CConfig(name="test_lcd_i2c", i2c_address=0x27, i2c_bus=1)
...
def test_8_mock_mode_behavior():
    ...
    config = LCDI2CConfig(name="mock_lcd_i2c", i2c_address=0x27, i2c_bus=1)
