...
def create_bff_router(app: FastAPI) -> tuple[APIRouter, ACSIClient]:
    ...
    async def api_reconfig_connection(request: TLSConnectionCreateConfigRequest):
        ...
        try:
            rti_so._log_action(f"Reconfig connection request: connection={request.connection_name}, enable_tls={request.enable_tls}", "debug")
            ...
            rti_so._log_action(f"TLS version determined: {tls_version} from request: {request.tls_version}", "debug")
            rti_so._log_action(f"Reconfiguring connection with TLS version: {tls_version} from request: {request.tls_version}", "debug")
            if request.ws_mode.lower() == "passive":
                rti_so._log_action("Reconfiguring passive endpoint with TLS", "debug")
                rti_so._log_action(f"Reconfiguring passive endpoint with TLS: {request.enable_tls}", "debug")
                ...
                if endpoint is not None and hasattr(endpoint, '_connect_task'):
                    ...
                    if connect_task and not connect_task.done():
                        rti_so._log_action("Cancelling existing connection task", "debug")
                        connect_task.cancel()
                ...
                rti_so._log_action("Invoking reconfigure_endpoint on runtime loop", "debug")
                ...
                rti_so._log_action(f"Endpoint status: {rti_so.runtime.endpoint._is_endpoint_running}", "debug")
    ...
    async def api_reconfig_oauth(request: OAUTHCreateConfigRequest):
        ...
        try:
            rti_so._log_action(f"Reconfig OAuth request: enable={request.enable_oauth}, connection={request.connection_name or 'unknown'}", "debug")
            ...
            if token_endpoint_from_req and not token_issuer_url:
                ...
                rti_so._log_action("Extracted token_issuer from token_endpoint URL", "debug")
            ...
            if request.ws_mode.lower() == "passive":
                ...
                rti_so._log_action("Invoking reconfigure_oauth on runtime loop", "debug")
                ...
                if request.enable_oauth:
                    rti_so._log_action("Waiting for OAuth endpoint to be running", "debug")
                    ...
                    rti_so._log_action(f"OAuth endpoint status: {rti_so.runtime.endpoint._is_endpoint_running}", "debug")
    ...
    async def api_model(request: ModelRequest):
        ...
        try:
            ...
            rti_so._log_action(f"HTTPException raised in api_model, re-raising: {e}", "debug")
            raise
        except Exception as exc:
