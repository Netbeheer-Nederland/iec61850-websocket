...
class StartRequest(BaseModel):
    """Request body for starting the ACSI Server WebSocket Passive."""
    host: str = Field(
        default="0.0.0.0",
...
def create_bff_router(
    ...
    @router.get(
        ...
        description="Retrieves the current operational status of the ACSI Server WebSocket Passive.",
    ...
    @router.post(
        ...
        description="Starts the ACSI Server WebSocket Passive on the specified host and port. Only 'server' mode is supported.",
    ...
    def api_start(request: StartRequest):
        """Start the ACSI Server WebSocket Passive.
    ...
    @router.post(
        "/stop",
        summary="Stop Server",
        description="Stops the currently running ACSI Server WebSocket Passive.",
    ...
    def api_stop():
        """Stop the ACSI Server WebSocket Passive.
...
def create_fastapi_app(factory_dir: Optional[Path] = None) -> FastAPI:
    ...
    app = FastAPI(
        title="ACSI Server WS Passive",
        description="Backend for Frontend (BFF) endpoint providing REST API for ACSI Server control. "
                    "This service manages ACSI Server WebSocket Passive lifecycle, IED models, data access, "
        ...
        openapi_tags=[
            {
                "name": "Server Control",
                "description": "Start, stop, and manage the ACSI Server WebSocket Passive lifecycle"
            },
            {
                "name": "Model",
