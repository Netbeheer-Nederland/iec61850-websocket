...
def _process_report_message(report: Dict[str, Any]) -> None:
    ...
    try:
        ...
        for entry_data in entry_data_list:
            ...
            report_updates.append({"dataRef": data_ref, "values": values, "timestamp": time.time()})
...
def start_connection_in_background(
    url: str,
    port: int,
    cp: Any,
    ...
    async def _connect():
        try:
                ...
                if enableTLS is True:
                    if mode == "active":
                        ...
                        if certificate != "" and private_key != "":
                            ...
                            tls_config = TLSConfiguration(
                                is_ws_server=True,
                                cert_path="ws_server_cert.pem",
                                key_path="ws_server_key.pem",
                            )
            ...
            ws_endpoint = WebSocketEndpoint(
                is_direct=endpoint_is_direct,
                tls_config=tls_config,
                oauth_enable=enable_oauth,
                cert_endpoint=certificate_url,
            ...
            if mode == "passive":
                ...
                if not token_refresh:
                    ...
                    task_1 = asyncio.create_task(ws_endpoint.start("active", url, port, cp, access_token=access_token))
...
def _ensure_connection(
    timeout: int = 10,
) -> Tuple[Any, WebSocketEndpoint, WebSocketInfo]:
...
def _list_logical_nodes(ld_inst: str, client: Any, ws_info: WebSocketInfo, timeout: int = 10) -> List[str]:
    ...
    try:
        ln_list = _invoke_async(
            client.get_logical_device_directory(ld_inst, ws_info, None, None),
            timeout=timeout,
        )
...
def _get_ln_details(
    ld_inst: str, ln_inst: str, client: Any, ws_info: WebSocketInfo, timeout: int = 10
) -> Dict[str, Any]:
    ...
    def _call(mode):
        try:
            return _invoke_async(
                _invoke_ln_directory(client, ws_info, ld_inst, ln_inst, mode),
                timeout=timeout,
            )
        except Exception:
            return None
...
def api_do_definition(ld_inst, ln_inst, do_path):
    ...
    try:
        ...
        log_action_end(
            aid,
            success=True,
            extra_detail={"subDO": len(sub_sdos), "DA": len(data_attrs)},
        )
...
def api_write_value():
    ...
    try:
        ...
        print(
            f"[Write Value] objRef={obj_ref}, fc={fc}, value={value} (type={type(value).__name__}), dataType={data_type}"
        )
...
def api_get_fcs():
    ...
    try:
        ...
        try:
            ...
            result = _invoke_async(client.get_data_directory(obj_ref, ws_info), timeout=10)
            # If result is an error string, attempt direct data definition and derive FCs
            if isinstance(result, str):
                dd = _invoke_async(client.get_data_definition(obj_ref, ws_info, None, None), timeout=10)
                if isinstance(dd, dict):
                    fcs_from_dd = sorted(
                        {
                            (da.get("fc") or "").lower()
                            for da in dd.get("dataAttributes", [])
                            if isinstance(da, dict) and da.get("fc")
                        }
                    )
                    if fcs_from_dd:
                        result = fcs_from_dd
...
def control_select():
    ...
    try:
        ...
        fut = asyncio.run_coroutine_threadsafe(client.select(obj_ref, websocket_info, None, None), loop)
    ...
    try:
        ...
        oper_val = {
            ...
            "origin": {
                "orCat": (
                    origin["orCat"]
                    if isinstance(origin["orCat"], str)
                    else origin_category_map.get(origin["orCat"], "bayControl")
                ),
        ...
        fut = asyncio.run_coroutine_threadsafe(client.operate(oper_val, websocket_info, None, None), loop)
        result = fut.result(timeout=30)
...
def control_cancel():
    ...
    try:
        ...
        fut = asyncio.run_coroutine_threadsafe(client.cancel(obj_ref, websocket_info, None, None), loop)
        result = fut.result(timeout=30)
