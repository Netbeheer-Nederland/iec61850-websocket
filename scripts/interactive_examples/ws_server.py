            ...
            while True:
                try:
                    ...
                    if function_name == "get_server_directory":
                        ...
                        ln_refs = await ep_wsServer.client_list[0].get_logical_device_directory(args[0], websocket_info, None,
                                                                                                None)
                        print("LN list: ", ln_refs)
                    elif function_name == "get_logical_node_directory":
                        ln_directory_items = await ep_wsServer.client_list[0].get_logical_node_directory(args[0], args[1],
                                                                                                      args[2],
                                                                                                      websocket_info,
                                                                                                      None,
                                                                                                      None)
                        ...
                        da_def = await ep_wsServer.client_list[0].get_data_definition(args[0],
                                                                                      websocket_info,
                                                                                      None, None)
                        print(da_def)
                    elif function_name == "get_data_values":
                        data_val = await ep_wsServer.client_list[0].get_data_values(args[0], args[1], args[2], websocket_info,
                                                                           None, None)
                        ...
                        values = {"ref": args[0], "ctlVal":('structure', {'data': [('structure', {'data': [(args[1], args[2])]})]}),
                        "origin": {
                       ...
                       "t" : get_now_time()
                        ...
                        set_data_val_result = await ep_wsServer.client_list[0].set_data_values(args[0], args[1],
                                                                           value, websocket_info,
                                                                           None, None)
                        print("set data values result: ", set_data_val_result)
                    elif function_name == "get_dataset_directory":
                        ds_directory = await ep_wsServer.client_list[0].get_dataset_directory(args[0], args[1],
                                                                         args[2], websocket_info,
                                                                         None, None)
                        print("dataset directory: ", ds_directory)
                    elif function_name == "get_BRCB_values":
                        brcb_val = await ep_wsServer.client_list[0].get_BRCB_values(args[0], websocket_info,
                                                                         None, None)
                        print("brcb value: ", brcb_val)

                    elif function_name == "get_URCB_values":
                        urcb_val = await ep_wsServer.client_list[0].get_URCB_values(args[0], websocket_info,
                                                                         None, None)
                        print(urcb_val)

                    elif function_name == "get_dataset_values":
                        ds_values =  await ep_wsServer.client_list[0].get_dataset_values(args[0], args[1], args[2],  websocket_info,
                                                                         None, None)
                        ...
                        brcb = IEC61850Client.ClientReportControlBlock(args[0],
                                                                       True)
                        ...
                        set_brcb_res = await ep_wsServer.client_list[0].set_BRCB_values(brcb, websocket_info,
                                                                                        None, None)
                        ...
                        urcb = IEC61850Client.ClientReportControlBlock(args[0],
                                                                       False)
                        #key, value = args[1].split("=", 1)
                        key, value = next(iter(kwargs.items()))
                        ...
                        set_urcb_res = await ep_wsServer.client_list[0].set_URCB_values(urcb, websocket_info,
                                                                                        None, None)
